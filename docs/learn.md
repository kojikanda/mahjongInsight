...
今回使用するDBは以下の2種類を利用する。

### 1. Neo4j（グラフデータベース）の役割

得意技：「点と線のつながり」から、複雑な関係性や理由を解き明かす

Neo4jは、「Aという事象」と「Bという結果」がどう繋がっているかという「関係」を保存します。

- 麻雀での活用例:
  - 「放銃（結果）」←（原因）ー「安牌不足」←（要因）ー「手広いイーシャンテン維持」
  - 「この特定の打ち手は、赤5が切れた後に必ずと言っていいほど跨ぎで待っている」といったパターンの構造化。

- なぜ使うか:
  「似たもの探し」だけでは分からない、「なぜそうなったか」という論理的なつながりをAIが辿れるようにするためです。

### 2. Vector DB（ベクトルデータベース）の役割

得意技：「なんとなく似ているもの」を、膨大なデータから一瞬で見つける

Vector DBは、文章や牌譜を「意味空間上の座標」として保存します。

- 麻雀での活用例:
  - 「今の自分の配牌に似た状況の牌譜を、過去1,000戦から探して」
  - 「『リーチに対して無筋を勝負すべきか』という議論をしている戦術記事を検索して」

- なぜ使うか:
  - 「キーワード一致」ではなく、文脈や状況の「ニュアンスの近さ」で検索できるからです。これが「RAG（検索拡張生成）」のベースになります。

## ■ なぜ2つのDBを組み合わせる必要があるのか（GraphRAGという考え方）

最近のAI開発では、この2つを組み合わせる 「GraphRAG」 という手法が最強とされています。

- Vector DBだけの場合:
  AIは「似たような過去の場面」をいくつか持ってこれますが、それらがどう関連しているかまでは理解できません。回答が「断片的」になりがちです。

- Neo4jを組み合わせる場合:
  1. まずVector DBで、今の状況に似た「知識の断片」を見つけます。
  2. 次にNeo4jで、その断片の「周辺にある関係性（なぜその打牌が選ばれたか、その後の結末はどうだったか）」を芋づる式に手繰り寄せます。

### ◯麻雀アプリでのイメージ

ユーザー：「この3段目のリーチ、押すべきだった？」

1. Vector DB: 過去の似たような「3段目・無筋押し」の場面を5つ持ってくる。
2. Neo4j: その5つの場面の背後にある「点数状況」「自分の順位」「相手の癖」との繋がりを解析。
3. AIの回答: 「過去の似た場面では80%放銃していますが、今回のNeo4jの分析によると、相手は直前に安全牌を入れ替えており、待ちが絞れているため、押しは正当化されます」
...
データやデータ間の関係がデータレコードごとに定義されるネイティブグラフDBです。<br>
...
- ノード: データ。ラベルとプロパティという概念を持つ。図では丸で表現する。
- リレーション: 関係(エッジ)。図では矢印で表現する。
- ラベル: そのノードが何であるか(どのグループに属すか)を表すRDBでいうテーブルのようなもの
- プロパティ: そのノードが実際に保持する情報
...
```
(Round {id: "E1-0"}) --[:HAS_INCIDENT]--> (Incident {turn: 11})
(Incident) --[:TYPE_OF]--> (Tactics {name: "牌効率"})
(Incident) --[:EVALUATED_BY]--> (MortalResult {ev_diff: 0.38})
```

#### ① ノード (Node)

括弧 ( ) で囲まれている部分です。実体や概念を表します。

- 例: (Round), (Incident), (Tactics)
- ラベル: Round や Tactics はノードの種類（クラスのようなもの）を指します。

#### ② プロパティ (Property)

中括弧 { } で囲まれている部分です。ノードやリレーションに紐付く「詳細データ」です。

- 例: id: "E1-0", turn: 11, name: "牌効率"
- 役割: 検索のキーにしたり、後で統計（例：5巡目以降のミスだけ抽出）を出すために使います。

#### ③ リレーションシップ (Relationship)

- 矢印 --[: ]--> で表現される部分です。データ同士の「関係性」を定義します。
- 例: [:HAS_INCIDENT], [:TYPE_OF]

#### ・スキーマの詳細

スキーマを、人間の言葉に翻訳するとこうなります。

1. (Round) --[:HAS_INCIDENT]--> (Incident)

「東1局0本場（Round）」という試合の単位が、「11巡目の出来事（Incident）」を持っている。

2. (Incident) --[:TYPE_OF]--> (Tactics)

「11巡目の出来事」は、「牌効率（Tactics）」という戦術カテゴリに属している。

3. (Incident) --[:EVALUATED_BY]--> (MortalResult)

「11巡目の出来事」は、「Mortal（MortalResult）」によって期待値差0.38と評価された。
...
→つまり、こういった「状況（Situation）」の整理をしたい。
